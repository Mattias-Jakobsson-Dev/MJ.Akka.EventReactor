...
public class MultiSourceEventReactorEventSource(params IEventReactorEventSource[] sources)  : IEventReactorEventSource
{
    public Source<IMessageWithAck, NotUsed> Start(CancellationToken cancellationToken)
    {
        return sources.Length switch
        {
            ...
            1 => sources[0].Start(cancellationToken),
            _ => Source.Combine(sources[0].Start(cancellationToken), sources[1].Start(cancellationToken), i => new Merge<IMessageWithAck>(i),
                sources.Skip(2).Select(x => x.Start(cancellationToken)).ToArray())
        };
    }
}